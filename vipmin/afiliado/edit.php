...
if ( $_POST && $id==$_POST['id'] && $partner) {
	...
	$up_array = array(
			...
			'image', 'image1', 'image2', 'longlat','chavesms',  'bairro', 'cep', 'estado', 'cidade','numero','descricao','bannerparceiro'
			);
	...
	if ( $flag ) {
		Session::Set('notice', 'Dados do website afiliado modificado com sucesso');
		redirect( WEB_ROOT . "/vipmin/afiliado/edit.php?id={$id}");
	}
	Session::Set('error', 'Falha na modificação dos dados do website afiliado');
	$partner = $_POST;
}

include template('manage_afiliado_edit');
