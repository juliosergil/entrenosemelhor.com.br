...
if(!empty($session)) {
	...
	if(!empty($user)){
		# We have an active session, let's check if we have already registered the user
			$query 	= mysqli_query(DB::$mConnection,"SELECT * FROM user WHERE AND fb_userid = ". $user[0]['uid']);
			...
			if(empty($result)){
				...
				$query 	= mysqli_query(DB::$mConnection,$sql);
				
				$query 	= mysqli_query(DB::$mConnection,"SELECT * FROM users WHERE id = " . mysqli_insert_id(DB::$mConnection));
				$result = mysqli_fetch_array($query);
			}		
