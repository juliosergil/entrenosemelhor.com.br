...
class Pager{
	...
	private function genURL( $param, $value ){
		global $stringzone,$stringidcategoria, $ROOTPATH;
		...
		if($_GET['id_category'] !=0 && isset($_GET['id_category']) && !(empty($_GET['id_category']))) {	
			...
			$sts_cat = "&id_category=".$id_category;
		}

		if(isset($_GET['state']) && !(empty($_GET['state']))) {	
			$state = strip_tags($_GET['state']);
			$sts_state = "&state=".$state;
		}	
	 ...
	 // return $this->script . '?' . http_build_query($valueArray) . $stringzone . $stringidcategoria . $ordena . $city_id . $partner_id; 
	 
	 if(!strpos($_SERVER["REQUEST_URI"],"vipmin") and !strpos($_SERVER["REQUEST_URI"],"adminanunciante")){
		return  $ROOTPATH . '/' . '?page=search&' .  http_build_query($valueArray) . $stringzone . $sts_cat . $sts_state . $sts_city . $ordena; 
	 }
	 else{
		return $this->script . '?' . http_build_query($valueArray) . $stringzone . $stringidcategoria . $ordena . $city_id . $partner_id; 
	  } 
