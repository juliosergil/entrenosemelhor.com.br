...
class DB
{
	...
	static public $mConnection = null;
	...
	function __construct()
	{
		...
		self::$mConnection = mysqlI_connect( $host, $user, $pass,$name );
     	//throw new Exception("Connect failed: " . mysqli_error(self::$mConnection));
		
		// IMPORTANTÍSSIMO: utf8mb4 para suportar emojis
		@mysqli_set_charset(self::$mConnection, 'utf8mb4');
		@mysqli_query(self::$mConnection, "SET NAMES utf8mb4 COLLATE utf8mb4_unicode_ci");
		@mysqli_query(self::$mConnection, "SET CHARACTER SET utf8mb4");
	...
	static public function Close()
	{
		...
		{
			//@mysqli_close(DB::$mConnection self::$mConnection );
		}
	...
	static public function Query( $sql )
	{
		...
		if ( self::$mDebug or $INI['option']['debug_sql']=="Y" )
		{
			echo $sql; 
		}
		...
		{ 
			...
			self::$mError = mysqli_error(self::$mConnection);
		}
	...
	static public function NextRecord($query) {
		return array_change_key_case(mysqli_fetch_assoc($query),CASE_LOWER);
	}
	...
	static public function LimitQuerySubs($table, $options=array(),$city_id)
	{
		global $INI;
		...
		if( $city_id == 0 ){ 
			$sql = "SELECT * FROM `subscribe` $order $limitation";
		}
		else{
			$sql = "SELECT * FROM `subscribe` where city_id = $city_id  $order $limitation";
		}		
		return self::GetQueryResult( $sql, $one, $cache);
	}	
	
	static public function LimitQueryCredito($table, $options=array())
	{
		return; 
	}
