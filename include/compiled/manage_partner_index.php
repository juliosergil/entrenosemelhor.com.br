				...
				<div class="option_box">
					 <div class="top-heading group"> <div class="left_float"><h4>Parceiros</h4></div> 
						...
						<div style="padding: 10px;">
							<ul id="log_tools"> <li id="log_switch_referral"><a title="Consultar pedidos pagos com saldo de crédito" href="/vipmin/parceiro/edit.php">Novo Parceiro</a></li> </ul> 
						</div>
					...
					<div class="sect" style="clear:both;">
                      ...
                      <!--  <? if(file_exists(WWW_MOD."/anunciante.inc")){?> <th width="20">Anúncios  <? } ?> -->
						...
						<tr <?php echo $index%2?'':'class="alt"'; ?> id="team-list-id-<?php echo $one['id']; ?>">
							...
							<? if(file_exists(WWW_MOD."/anunciante.inc")){ 
								if ($one['max_anuncios'] > 0) {
									$count = mysqli_fetch_row(mysqli_query(DB::$mConnection,"SELECT COUNT(id) AS Contagem FROM team WHERE partner_id = {$one['id']} AND usou_bonus = 'sim'"));
									$count = $count[0];
									$anuncios = "{$count} de {$one['max_anuncios']}";
								} else if ($one['max_anuncios'] == 0) {
									$anuncios = 'Ilimitado';	
								} else {
									$anuncios = 'Pago Indv.';	
								}
