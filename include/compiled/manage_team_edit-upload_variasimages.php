	...
	<div id="content" class="clear mainwide">
        <div class="clear box"> 
            <div class="box-content">
						...
						<div class="left_float"><h4>Informações gerais do produto <?=$team['id']?></h4></div>
							<div class="the-button" style="width:543px;"> 
								...
								<div style="float:left;">
									<button id="run-button" class="input-btn doUpdate" type="button">
										<div id="spinner-text"  >Salvar</div>
									</button>
								</div>
								<div style="float:left;">
									<button  onclick="javascript:location.href='<?=$ROOTPATH?>/vipmin/category/index.php?zone=group'"  id="run-button" class="input-btn" type="button">
									<div id="spinner-text"  >Cadastrar categorias</div>
									</button>
								</div> 
								<div style="float:left;">
									<button  onclick="javascript:location.href='index.php'"  id="run-button" class="input-btn" type="button">
										<div id="spinner-text"  >Listar Anúncios</div>
									</button>
								</div>
				...
				<div class="option_box">
					<div class="top-heading group"> 
						<div class="left_float"><h4>Adicionar imagens ao anúncio - No máximo 6 imagens por anúncio</h4></div>
					</div> 
					<div id="container_box">
						<div id="option_contents" class="option_contents">  
							<div class="form-contain group">
								<!-- =============================   coluna esquerda   =====================================-->
								<div class="starts">  
									<div style="clear:both;"class="report-head">Para escolher várias imagens, mantenha a tecla CTRL pressionada e clique nas imagens desejadas  
										<span class="cpanel-date-hint"><span id="dimensao"></span>  </span> 
									</div>
									<div class="group">
										<input type="file" multiple name="tempImg" id="tempImg" class="format_input" style="border: 1px solid #b7c4d8;height: 30px;border-radius: 3px;width: 80%;" />  
									 </div> 
								</div>
								<!-- ============================= // fim coluna esquerda // =====================================-->
								<!-- ============================= // coluna direita // =====================================-->
											...
											<?php
												...
												if($team['id']){
													...
													foreach ($images as $img) {
														...
														echo "<img src='".$ROOTPATH."/media/team/".$img['img']."' />";
														echo "</div>";
													}
												}
											?>
...
function saveTeamData(){
	...
	$.ajax({
		   type: "POST",
		   url: "<?php echo $INI['system']['wwwprefix']?>/vipmin/team/edit.php",
		   data: fdObj, 
           dataType: 'json',

		   success: function(data){
		   		if(data['status'] == 1){	
		   			 saveTeamImages(data['id']);
        ...
        success: function(data){
        	if(data['status'] == 1){
				alert("Imagens salvas com sucesso!");
   				$(".animationload").hide();
   				window.location = "<?php echo $ROOTPATH; ?>/vipmin/team/index.php";
   			}else{
...
<style type="text/css">
	...
	.preview-thumbs-container img{
		max-width: 100%;
		height: 100px;
		margin-bottom: 10px;	
	}

	.preview-thumbs-container:hover > .fa-times-circle{
		display: block;
		cursor: pointer;
	}
	...
	.helpbox_container{
		...
		padding: 24px;
		border-radius: 5px;
		box-shadow: 2px 6px 10px #a7b6c2;
	}
