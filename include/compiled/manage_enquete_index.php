...
if(!empty($_GET['id']) && empty($_POST)){ 
		...
		$show 					 =  mysqli_query(DB::$mConnection,$sql);
...
if(!empty($_POST) && empty($_GET['id'])){
	...
	if(in_array($extensao,$array_formato)){ 
		...
		mysqli_query(DB::$mConnection,$qry) or die('<script>alert("Erro: '.mysqli_error(DB::$mConnection).'");</script>');
		...
		<div class="box clear"> 
            <div class="box-content">
                ...
                if(!empty($_GET['show'])){
                	...
                	$show 	=  mysqli_query(DB::$mConnection,$sql);
                	$show_n	=  mysqli_num_rows($show);
                	
                	if( $show_n > 0){
                		
                		
                		$i=0;
						while($show_ = mysqli_fetch_assoc($show)){
							...
							<div id="option_contents" class="option_contents">  
								<div class="form-contain group"> 
									<div class="text_area">   
										...
										<div class="group">
											<input type="text" name="nome"  maxlength="162" id="nome" class="format_input ckeditor" value="<?php echo  $team['largura'] ; ?>" /> 
										</div>
									<div style="clear:both;"class="report-head">Imagem:  <span class="cpanel-date-hint"></span></div>
									<div class="group">
										<input type="file" style="border: 1px solid #C1D0D3; color: #666666; width: 86%;" name="enviashow"  id="enviashow" class="format_input ckeditor" />     
									</div> 
