...
elseif ( 'republica' == $action) {   
		...
		republicar_anuncio($idanuncio);
		Session::Set('notice',  "O anúncio  $idanuncio foi republicado com sucesso"); 
		return $idanuncio;
}
elseif ( 'saveImages' == $action) {   
	$images = $_POST['images'];
	$id = $_POST['id'];
	$rt = insertTeamImages($images, $id);
	$data = json_encode(['status' => $rt]);
	echo $data;
	exit;
}
...
elseif ( 'duplicar' == $action) {
	$id = $_REQUEST['id'];
	$sql = "INSERT INTO team ( republicacao,pontos,pontosgerados,idpacote,`processo_compra`, `categoria_agrupi`, `categoria_valejunto`, `cidade_valejunto`, `categoria_apontaofertas`, `cidade_apontaofertas`, `categoria_dsconto`, `minimo_pessoa`, `user_id`, `layout`, `title`, `summary`, `city_id`, `group_id`, `partner_id`, `system`, `team_price`, `preco_comissao`, `market_price`, `product`, `condbuy`, `per_number`, `min_number`, `max_number`, `now_number`, `pre_number`,`flv`, `mobile`, `credit`, `card`, `fare`, `farefree`, `bonus`, `address`, `detail`, `systemreview`, `userreview`, `notice`, `express`, `delivery`, `state`, `conduser`, `buyonce`, `team_type`, `sort_order`, `expire_time`, `begin_time`, `end_time`, `reach_time`, `close_time`, `seo_title`, `seo_keyword`, `seo_description`, `posicionamento`, `semhtmldescricao`, `semhtmlregulamento`, `manterdimensao`, `metodo_pagamento`, `bonuslimite`, `retornoparticipe`, `url_comprar`, `marcadagua`, `maisinformacoes`, `estatica_home`, `estatica_direita`, `estatica_detalhe`, `estatica_recentes`, `frete`, `ceporigem`, `peso`, `altura`, `comprimento`, `largura`, `valorfrete`, `fretegratuito` ) 
	SELECT republicacao,pontos,pontosgerados,idpacote,`processo_compra`, `categoria_agrupi`, `categoria_valejunto`, `cidade_valejunto`, `categoria_apontaofertas`, `cidade_apontaofertas`, `categoria_dsconto`, `minimo_pessoa`, `user_id`, `layout`, `title`, `summary`, `city_id`, `group_id`, `partner_id`, `system`, `team_price`, `preco_comissao`, `market_price`, `product`, `condbuy`, `per_number`, `min_number`, `max_number`, `now_number`, `pre_number`,  `flv`, `mobile`, `credit`, `card`, `fare`, `farefree`, `bonus`, `address`, `detail`, `systemreview`, `userreview`, `notice`, `express`, `delivery`, `state`, `conduser`, `buyonce`, `team_type`, `sort_order`, `expire_time`, `begin_time`, `end_time`, `reach_time`, `close_time`, `seo_title`, `seo_keyword`, `seo_description`, `posicionamento`, `semhtmldescricao`, `semhtmlregulamento`, `manterdimensao`, `metodo_pagamento`, `bonuslimite`, `retornoparticipe`, `url_comprar`, `marcadagua`, `maisinformacoes`, `estatica_home`, `estatica_direita`, `estatica_detalhe`, `estatica_recentes`, `frete`, `ceporigem`, `peso`, `altura`, `comprimento`, `largura`, `valorfrete`, `fretegratuito` FROM team
	WHERE id = ".$id ;
	$rs = mysqli_query(DB::$mConnection,$sql);
	Session::Set('notice', "Oferta {$order['id']} duplicada com sucesso. Não esqueça de fazer o upload das fotos. Atenção: Só duplique ofertas do mesmo tipo. Por exemplo, não duplique ofertas do tipo promocional para ofertas do tipo normal.");
	json(null, 'refresh');
}
	...
	if($rs){
		...
		Session::Set('notice', 'Pedido '.$id.' não excluído. '.$sql. ' '. mysqli_error(DB::$mConnection));
	}
	json(null, 'refresh');
}
	...
	if($idmodelo!="" and $idmodelo!="0" and $idmodelo!="00"){
		...
		$rs  = mysqli_query(DB::$mConnection,$sql );
		echo mysqli_insert_id(DB::$mConnection);
	}
	if(!$rs){
		//echo  'Erro em salvar este modelo '.$sql. ' '. mysqli_error(DB::$mConnection);
	 } 	 
	 sleep(1);
}
	...
	if($idmodelo!="" and $idmodelo!="0" and $idmodelo!="00"){
	    echo $sql  = "update page  set value = '".$mensagem."', titulo = '$titulo', datamodificacao = '".date("Y-m-d H:i:s")."',status = '$status' where id = '$idmodelo'";
		$rs  = mysqli_query(DB::$mConnection,$sql );
		if($rs){ 
			echo "true";
		}  
		...
		$rs  = mysqli_query(DB::$mConnection,$sql );
		$sql  = "insert into page (id,value,data_criacao,titulo,status) values('$id','$mensagem','".date("Y-m-d H:i:s")."','$titulo','$status')";
		$rs  = mysqli_query(DB::$mConnection,$sql );
		echo $id ;
	}
	if(!$rs){
		 echo  'Erro em salvar esta página. Por favor, tente novamente. '.$sql. ' '. mysqli_error(DB::$mConnection);
	 } 
	  sleep(1);
}
	...
	//$rs  = mysqli_query(DB::$mConnection,$sql );		
	//if(!$rs){		
	//	Util::log( mysqli_error(DB::$mConnection) . " ". $sql);	
	//} 
	...
	if($rs){
		...
		Session::Set('notice', 'O usuário id ('.$id.') não foi excluído. '.$sql. ' '. mysqli_error(DB::$mConnection));
	}
	json(null, 'refresh');
}
...
elseif ( 'deleteestado' == $action) {
		
		Table::Delete('estados', $id);
		Session::Set('notice', "estado {$id} excluído com sucesso");
		json(null, 'refresh');
	
}
else if ( 'delall' == $action ) {
 
	$tabela = $_REQUEST['tabela']; 
	Util::log($id ." - deletando todos  : ".$tabela );
   
	$sql  = "delete from $tabela";
	$rs  = mysqli_query(DB::$mConnection,$sql ); 
	Session::Set('notice', 'Registros excluídos com sucesso.');
 
	json(null, 'refresh');
}

else if ( 'userremovewna' == $action ) {
	$id = $_REQUEST['id']; 		
	$user = Table::Fetch('user', $id);	
	$partner = Table::Fetch('partner', $user['email'], 'username');	
	Util::log( "DELETANDO USER ".$id. " email ". $user['email']. " partner ".$partner['username']. " partner id ".$partner['id']);
...
*/
   ...
   ****************************/
	...
	//json(null, 'refresh');
	redirect($ROOTPATH."/vipmin/misc/subscribe.php");
}
	...
	if ($partner && $count==0) {
		...
		Table::Delete('user', $partner['username'], 'username');
